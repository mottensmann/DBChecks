...
ReadFormTools <- function(path = "data", file = "form561", type = ".csv") {
  data <- read.csv(DBChecks::file.select(path, file, type),
                sep = ";", encoding = "UTF-8", header = F,
                skip = 1, col.names = c(
                  "ID", "Title", "Name" ,"Email", "Species",
                  "Condition", "Color", "Caption", "Another.Caption", "Ring",
                  "Where", "Date", "When", "Comments", "History",
...
write.csv2(df, paste0(path, "ResightsFormTools", "-",Sys.Date(),".csv"),
          quote = F, row.names = F)
print("Done")
}
